...
async def pub_(bot, message):
    ...
    sts.add(time=True)
    # SPEED OPTIMIZATION: Reduced sleep times and increased batch size
    sleep = 0.1 if _bot['is_bot'] else 3  # Reduced from 10 seconds
    await msg_edit(m, "<code>processing...</code>") 
        ...
        try:
          ...
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("skip"), filters=filter, max_size=max_size):
                ...
                if forward_tag:
                   ...
                   # SPEED OPTIMIZATION: Increased batch size from 100 to 200
                   if ( notcompleted >= 200 
                        or completed <= 200): 
...
async def complete_time(total_files, files_per_minute=30):
    ...
    return time_format                   if user_have_db:
                      await user_db.drop_all()
                      await user_db.close()
                   return
                if pling %20 == 0: 
